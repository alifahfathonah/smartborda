         ...
         <table class="table table-bordered no-footer">
            ...
            <tbody>
               <tr align="center" style="background-color: #ffefef">
                  ...
                  <?php $idperiode = (!$this->uri->segment(3)) ? cekPeriodeHasil() : cekPeriodeHasil($this->uri->segment(3)); ?>
                  <?php foreach ($kriteria as $kr) :
                     $min = minPoint($kr['id_kriteria'], $idperiode);
                  ?>
                     <td><?= $min['minPoint'] ?></td>
                  <?php endforeach ?>
               </tr>
               <tr align="center" style="background-color: #e8fff4">
                  ...
                  <?php $idperiode = (!$this->uri->segment(3)) ? cekPeriodeHasil() : cekPeriodeHasil($this->uri->segment(3)); ?>
                  <?php foreach ($kriteria as $kr) :
                     $max = maxPoint($kr['id_kriteria'], $idperiode);
                  ?>
                     <td><?= $max['maxPoint'] ?></td>
                  <?php endforeach ?>
               </tr>
